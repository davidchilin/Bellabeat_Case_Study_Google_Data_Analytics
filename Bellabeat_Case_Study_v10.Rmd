---
title: "Bellabeat Case Study (Fitness Tracker)"
author: "David Chilin"
date: "8/13/2021"
...
1.  Data Source: [FitBit Fitness Tracker Data LINK](https://www.kaggle.com/arashnic/fitbit) (CC0: Public Domain, dataset made available through Mobius): This Kaggle data set contains personal fitness tracker data from thirty-three FitBit users.
    The eligible Fitbit users consented to the submission of personal tracker data, including second/minute/hour/day-level output for physical activity (distance, steps), heart rate, and sleep monitoring.
...
```{r 'hourlyCalories', warning=FALSE, fig.height=4, indent = "      "}
hourlyCalories %>%
  select(-Id) %>%
  summary()
...
-   **hourlySteps, hourlyIntensities**:
    ...
    -   Consider redundant with hourlyCalories, *drop hourlySteps, hourlyIntensities*
...
sleepDay %>% group_by(Id) %>% count(Id, sort = TRUE) %>% print(n = 24)
    ...
    -   24 distinct users, 413 observations *possibly encourage users to use functionality*
...
weightLogInfo %>%
  ...
  count(Id)
    ...
    -   *Drop weightLogInfo due to limited number of users and observations, possibly encourage users to use functionality.*
...
dailyMerged <- left_join(dailyActivity, sleepDay, by = c("Id", "ActivityDate")) %>%
  mutate(ActivityDate = mdy(ActivityDate))

# Categorize hourlyCalories and lubridate ActivityHour
hourlyCalories <- hourlyCalories %>%
  mutate(ActivityHour = mdy_hms(ActivityHour),
         weekday = factor(wday(ActivityHour, label = TRUE)),
         week = factor(case_when(
           ...
           ActivityHour >= "2016-05-10" ~ 5), # week 5
           ordered = TRUE),
         CalorieCat = factor(case_when(
           Calories < 100 ~ "< 100",
...
The next set of boxplots give an overview of user metrics in Daily Sleep Hours, Restless Bed Time, Daily Calories Burned, and Daily Steps.
Users are designated Letters A-Z and Numbers 1 - 6.
Missing user boxplots are from NA observations (either non-existant or dropped).

```{r, ggplot, collapse=TRUE, fig.height=3, warning=FALSE, fig.show='hold'}
...
Daily Sleep Hours Plot shows 15 users averaging about `r dailyMerged %>% drop_na %>% summarise(mean(TotalMinutesAsleep)/60) %>% round(1)` hours of sleep. 
Restless Bed Time was pretty consistent with an average of `r drop_na(dailyMerged) %>% summarise(mean(RestlessBedTime)) %>% round(0)` minutes, with one user (User L) as an outlier averaging `r filter(dailyMerged,Id == unique(Id)[12]) %>% drop_na %>% summarise(mean(RestlessBedTime)) %>% round(0)` minutes, which coincidentally paired with the lowest average Daily Sleep Hours of `r filter(dailyMerged,Id == unique(Id)[12]) %>% drop_na %>% summarise(mean(TotalMinutesAsleep)/60) %>% round(1)` hours.

The plots of Daily Calories Burned and Daily Steps do somewhat follow a similar pattern as was indicated in the correlation heatmap with a value of 0.41. Number of steps varied quite a bit with a daily average and standard deviation of : `r dailyMerged %>% summarise(mean(TotalSteps), sd(TotalSteps)) %>% round(0)`.
Ultimately, these plots were added for practice and the extra color they bring to the document.



```{r Melt-Plots}
# Stacked barchart of User Activity Minutes and X-Y Scatter of Calories Burned
...
This combined plot of Average Activity Minutes and Calories Burned is mostly an exercise in presenting two datasets within a single plot using a second axis.
The collective Activity Minutes (not including Sedentary Activity Minutes) do seem to follow a similar trend to the values of Calories Burned.
...
dailyMerged <- dailyMerged %>%
  mutate(Id = factor(Id),
         week = factor(case_when(
           ...
           ActivityDate >= "2016-05-10" ~ 5), # week 5
           ordered = TRUE
  ))
...
```{r Weekly-Sleep, warning=FALSE, fig.height=2}
dailyMerged %>%
...
```{r Kmeans, fig.height=2, warning=FALSE, message=FALSE}
...
The four clusters have: `r km$size` users with within cluster sum of squares `r round(km$betweenss/km$totss*100, 1)`%. A summary of each cluster's average metric values is presented in table below.
...
Based on PCA by weight, it seems the most distinguishing metrics are *TotalSteps, VeryActiveMinutes, LightlyActiveMinutes, Calories*, and *Sedentary Minutes*. The last plot shows the grouping of users by color overlayed with the first two PCA dimensions. As might of been expected the users were grouped by activity, falling into activity categories: *Very, Fairly, Lightly,* and *Sedentary*. A possible good addition would be an analysis of hourlyCalories to categorize by schedule, to determine users as having school/work schedule, post-work workouts, weekend athletes/loungers, etc.
...
1. *User calorie* schedule were presented in monthly/weekly/hourly heatmaps. From these heatmaps users can be categorized by weekly patterns:
...
2. *User fitness* was categorized into four groups using PCA. As the data used for the PCA is fitness data, the results appropriately categorized users fitness as: *Very, Fairly, Lightly*, and *Sedentary*. Based on PCA by weight, the most distinguishing metrics in order are:
    -   TotalSteps
    -   VeryActiveMinutes
    -   LightlyActiveMinutes
    -   Calories
